// ProjectEuler0011.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
constexpr char numbers[20][60] = { "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08",
...
int64_t get_max_horizontal_product(int num_terms) {
    ...
    for (const auto& row : grid) {
        auto first_iter = row.cbegin();
        auto last_iter = row.cbegin() + num_terms;

        while (last_iter != row.cend()) {
            int64_t product = std::accumulate(first_iter, last_iter, 1LL, std::multiplies<int64_t>());
...
int main()
{
    ...
    std::cout << get_max_horizontal_product(4) << std::endl;
    std::cout << get_max_vertical_product(4) << std::endl;
    std::cout << get_max_ne_diagonal_product(4) << std::endl;
    std::cout << get_max_se_diagonal_product(4) << std::endl;
